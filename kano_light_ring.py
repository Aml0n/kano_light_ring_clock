...
def turnOnLightsWithGradient(rangenum):
...
def hourbinary_to_light():
    ...
    for i, digit in enumerate(binary_str):
        if emmage == 'AM':
            if digit == '1':
                strip.setPixelColor(led_positions[i], NEYELLOW2)
            else:   
                strip.setPixelColor(led_positions[i], NEYELLOWDIM)
        if emmage == 'PM':
            if digit == '1':
                strip.setPixelColor(led_positions[i], NEPURPLE3)
            else:
                strip.setPixelColor(led_positions[i], NEPURPLE2DIM)
    if emmage == 'AM':
